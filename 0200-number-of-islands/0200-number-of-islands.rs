impl Solution {
    pub fn num_islands(grid: Vec<Vec<char>>) -> i32 {
        ...
        for &id in union_ids.iter() {
            let (r, c) = cell_id_to_row_col(id as u32, &grid);
            if grid[r][c] == '1' {
                total += 1;
            }
        }
...
fn to_cell_id(row: usize, col: usize, grid: &Vec<Vec<char>>) -> u32 {
    let col_size = grid[0].len();

    return (row * col_size + col) as u32;
}

fn cell_id_to_row_col(cell_id: u32, grid: &Vec<Vec<char>>) -> (usize, usize) {
    let col_size = grid[0].len() as u32;
